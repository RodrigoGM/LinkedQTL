...
library(snow)
lapply(list.files(path = "../", pattern = "3\\.R"), function(f) {
  source(paste("../", f, sep = ""))
})
...
load("../C-map.RData")
load("../seed1.RData")
qtl<- read.csv("../CF2-QTL_effects.csv")
...
complete.cross<- sim.cross.npheno(map, n.ind = nind, model = qtl.models[[i]],
  type = "f2", map.function = "haldane", error.prob = 0.009, missing.prob = 0.009)
write.cross(complete.cross, format = "csv", paste(i, "_C-F2", "_q", qtl. , "_nind", nind, "_nmar", nMar, sep = ""))
  }) ## end parLapply
} ## make.world

              ...
              keep.errorind, m, p, map.function) 
{
    ...
    for (i in 1:n.chr) {
        thedata <- qtl:::sim.bcg(n.ind, map[[i]], m, p, map.function)
        dimnames(thedata) <- list(NULL, mar.names[[i]])
        if (chr.type[i] != "X") 
            thedata <- thedata + qtl:::sim.bcg(n.ind, map[[i]], m, 
                                         p, map.function) - 1
    ...
    source("../src/Make5pheno.R")
    pheno <- make.5.pheno(n.ind)
    ...
    if (missing.prob > 0) {
        for (i in 1:n.chr) {
            ...
            geno[[i]]$data[sample(c(TRUE, FALSE), n.mar[i] * 
                                      n.ind, replace = TRUE, prob = c(missing.prob, 
                                                                 1 - missing.prob))] <- NA
            if (length(o) > 0) 
                geno[[i]]$data[, o] <- x
        }
